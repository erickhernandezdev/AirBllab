...
class NewProposalForm(forms.Form):
    ...
    price = forms.DecimalField(
        max_digits=20, decimal_places=2, min_value=0, label="Precio"
    )
    image = forms.ImageField(
        widget=forms.ClearableFileInput(attrs={"id": "image-input", "hidden": True}),
        required=False,
        label="Imagen",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if selected_type == "Alojamiento":
            self.fields["subtype"].choices = [
                (a.name, a.name) for a in AccommodationType.objects.all()
            ]

        elif selected_type == "Actividad":
            self.fields["subtype"].choices = [
                (a.name, a.name) for a in ActivityType.objects.all()
            ]

        elif selected_type == "Servicio":
            self.fields["subtype"].choices = [
                (s.name, s.name) for s in ServiceType.objects.all()
            ]
